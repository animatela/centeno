...
const form = useForm({
    ...
    number: props.reservation.number,
...
const updateVehicle = () => {
    form.put(route('reservations.update', { id: reservationId.value }), {
        preserveScroll: true,
        onSuccess: () => showSuccessMessage('Success', 'Your reservation was updated!')
    })
}
</script>
    ...
    <section>
        <header>
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                Actualizar Reserva {{ reservation.number }}
            </h2>
        ...
        <form @submit.prevent="updateVehicle" class="mt-6 space-y-6">
            <div>
                <InputLabel for="vehicle_id" value="Vehículo" />

                <SelectInput
                    id="vehicle_id"
                    class="mt-1 block w-full read-only:bg-gray-100 dark:read-only:bg-gray-700"
